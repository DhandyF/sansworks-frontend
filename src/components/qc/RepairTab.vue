    ...
    <Card variant="bordered">
      ...
      <template v-else>
        <Table :columns="columns" :rows="items" :per-page="15" showVerticalBorder>
          ...
          <template #taken_date="{ value }"><span class="whitespace-nowrap">{{ formatDate(value) }}</span></template>
          <template #deadline_date="{ value }"><span class="whitespace-nowrap">{{ formatDate(value) }}</span></template>
          <template #status="{ value }"><Badge :variant="statusBadge(value)" size="sm"><span class="whitespace-nowrap">{{ statusLabel(value) }}</span></Badge></template>
    ...
    <Modal v-model="showForm" :title="editing ? t('repairs.editRepair') : t('repairs.addRepair')" size="lg" :closeOnOverlay="false">
      <div class="space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div v-if="editing">
            <Input v-model="editing.tailor.name" :label="t('repairs.tailor')" disabled />
          </div>
          <SearchableDropdown v-else v-model="form.tailor_id" :options="tailors" :label="t('repairs.tailor')" :placeholder="t('repairs.selectTailor')" required />
          <div v-if="editing">
            <Input v-model="editing.brand.name" :label="t('repairs.brand')" disabled />
          </div>
          <SearchableDropdown v-else v-model="form.brand_id" :options="brands" :label="t('repairs.brand')" :placeholder="t('common.selectBrand')" required />
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div v-if="editing">
            <Input v-model="editing.article.name" :label="t('repairs.article')" disabled />
          </div>
          <SearchableDropdown v-else v-model="form.article_id" :options="articles" :label="t('repairs.article')" :placeholder="t('repairs.selectArticle')" :disabled="!form.brand_id" required />
          <div>
