...
async function handleSubmit() {
  ...
  try {
    ...
    if (!preOrderId) {
      formError.value = t('cuttingResults.invalidCombination')
      submitting.value = false
      return
    }
    const totalCutting = Number(form.value.total_cutting)
    if (sizeRemaining.value && totalCutting > sizeRemaining.value.available) {
      formError.value = t('cuttingResults.exceedsAvailable', { total: totalCutting, available: sizeRemaining.value.available })
      submitting.value = false
      return
    }
    const payload = {
  ...
  <div>
    <Card variant="bordered">
      ...
      <div v-else-if="items.length === 0" class="text-center py-12">
        <p class="text-surface-500">{{ t('cuttingResults.noResults') }}</p>
      </div>
      <Table v-else :columns="columns" :rows="groupedResults" expandable :per-page="15" showVerticalBorder>
    ...
    <Modal v-model="showForm" :title="editing ? t('cuttingResults.editCuttingResult') : t('cuttingResults.addCuttingResult')" size="lg" contentClass="h-[80vh]" :closeOnOverlay="false">
      <div class="space-y-4">
        <div v-if="formError" class="p-3 bg-red-50 border border-red-200 rounded-lg text-red-700 text-sm">{{ formError }}</div>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <SearchableDropdown v-model="form.brand_id" :options="brands" :label="t('common.brand')" :placeholder="t('common.selectBrand')" required />
          <SearchableDropdown v-model="form.pre_order_name" :options="preOrders" :label="t('common.preOrder')" :placeholder="preOrdersLoading ? 'Loading...' : (!form.brand_id ? t('cuttingResults.selectBrandFirst') : t('cuttingResults.selectPreOrder'))" :disabled="!form.brand_id || preOrdersLoading" required />
        </div>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <SearchableDropdown v-if="formSteps.showArticle" v-model="form.article_id" :options="preOrderArticles" :label="t('common.article')" :placeholder="t('cuttingResults.selectArticle')" required />
          <div v-else class="flex items-end"><p class="text-sm text-surface-400 pb-2">{{ t('cuttingResults.selectPreOrderFirst') }}</p></div>
          <SearchableDropdown v-if="formSteps.showSize" v-model="form.size_id" :options="preOrderSizes" :label="t('common.size')" :placeholder="t('cuttingResults.selectSize')" required />
          <div v-else class="flex items-end"><p class="text-sm text-surface-400 pb-2">{{ t('cuttingResults.selectArticleFirst') }}</p></div>
        </div>
        <div v-if="sizeRemaining" class="p-3 bg-surface-50 rounded-lg space-y-1 text-sm">
          <p><span class="font-medium">{{ t('cuttingResults.preOrderQty') }}:</span> {{ sizeRemaining.total_pcs }}</p>
          <p><span class="font-medium">{{ t('cuttingResults.alreadyCut') }}:</span> {{ sizeRemaining.cut_qty }}</p>
          <p><span class="font-medium">{{ t('cuttingResults.available') }}:</span> <span :class="sizeRemaining.available > 0 ? 'text-green-600' : 'text-red-600'" class="font-medium">{{ sizeRemaining.available }}</span></p>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Input v-model="form.total_cutting" :label="t('cuttingResults.totalCutting')" type="number" placeholder="0" required />
          <Input v-model="form.cutting_date" :label="t('cuttingResults.cuttingDate')" type="date" required />
        </div>
        <Input v-model="form.notes" :label="t('common.notes')" type="textarea" :placeholder="t('cuttingResults.optionalNotes')" />
      </div>
      <template #footer>
    ...
    <Modal v-model="showDeleteModal" :title="t('cuttingResults.deleteTitle')" size="sm" :closeOnOverlay="false">
