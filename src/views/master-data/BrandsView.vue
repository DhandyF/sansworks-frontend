...
const { items, loading, pagination, fetchData, addItem, editItem, deleteItem } = useMasterData('/brands', () => ({ search: search.value }))
...
function goToDetail(row) {
  router.push({ name: 'brand-detail', params: { id: row.id } })
}
...
<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      <div>
        <h1 class="text-2xl font-bold text-surface-900">{{ t('brands.title') }}</h1>
        <p class="mt-1 text-sm text-surface-500">{{ t('brands.description') }}</p>
      </div>
      <Button @click="openAddForm">{{ t('brands.addBrand') }}</Button>
    </div>

    <div class="mb-4">
      <Input v-model="search" label="" :placeholder="t('brands.searchPlaceholder')" />
    </div>
    ...
    <Card variant="bordered">
      ...
      <div v-else-if="items.length === 0" class="text-center py-12">
        <p class="text-surface-500">{{ t('brands.title') }}</p>
      </div>
      <template v-else>
        <Table :columns="columns" :rows="items" :pagination="pagination" clickable @row-click="goToDetail" @page-change="fetchData" showVerticalBorder>
    ...
    <Modal v-model="showForm" :title="editing ? t('brands.editBrand') : t('brands.addBrand')" size="md" contentClass="h-[80vh]" :closeOnOverlay="false">
      <form @submit.prevent="handleSubmit" class="space-y-4">
        <div v-if="formError" class="p-3 bg-red-50 border border-red-200 rounded-lg text-red-700 text-sm">{{ formError }}</div>
        <Input :model-value="form.name" @update:model-value="v => form.name = upper(v)" :label="t('common.name')" :placeholder="t('brands.brandName')" required />
        <Input :model-value="form.phone" @update:model-value="v => form.phone = upper(v)" :label="t('common.phone')" :placeholder="t('brands.phoneNumber')" />
        <Input :model-value="form.address" @update:model-value="v => form.address = upper(v)" :label="t('common.address')" type="textarea" :placeholder="t('brands.brandAddress')" />
        <div class="space-y-1">
    ...
    <Modal v-model="showDeleteModal" :title="t('brands.deleteTitle')" size="sm" :closeOnOverlay="false">
