...
const { items, loading, currentPage, totalItems, lastPage, fetchData, addItem, editItem, deleteItem } = useMasterData('/tailors')
...
<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      <div>
        <h1 class="text-2xl font-bold text-surface-900">Tailors</h1>
        <p class="mt-1 text-sm text-surface-500">Manage tailor information</p>
      </div>
      <Button @click="openAddForm">+ Add Tailor</Button>
    </div>
    ...
    <Modal v-model="showForm" :title="editing ? 'Edit Tailor' : 'Add Tailor'" size="md">
      <form @submit.prevent="handleSubmit" class="space-y-4">
        <div v-if="formError" class="p-3 bg-red-50 border border-red-200 rounded-lg text-red-700 text-sm">{{ formError }}</div>
        <Input :model-value="form.name" @update:model-value="v => form.name = upper(v)" label="Name" placeholder="Tailor name" required />
        <Input :model-value="form.phone" @update:model-value="v => form.phone = upper(v)" label="Phone" placeholder="Phone number" />
        <Input :model-value="form.address" @update:model-value="v => form.address = upper(v)" label="Address" type="textarea" placeholder="Tailor address" />
        <div class="space-y-1">
    ...
    <Modal v-model="showDeleteModal" title="Delete Tailor" size="sm">
