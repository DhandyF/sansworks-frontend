...
<body>
  <div class="slip">
    ...
    <table>
      ...
      <tbody>
        ${p.items.length === 0 ? `<tr class="empty-row"><td colspan="${colCount + 4}">${t('payslip.noProductionData')}</td></tr>` : rows}
        ${compensationRows}
      </tbody>
      <tfoot>
        <tr>
          ...
          <table class="w-full text-sm">
            ...
            <tbody>
              <tr v-if="payslip.items.length === 0">
                <td :colspan="payslip.columns.length + 4" class="px-4 py-8 text-center text-surface-400">{{ t('payslip.noData') }}</td>
              </tr>
              ...
              <tr v-for="comp in compensations" :key="comp.id" class="border-b border-surface-100 bg-green-50">
                ...
                <td :colspan="payslip.columns.length + 1" class="px-4 py-2.5"></td>
