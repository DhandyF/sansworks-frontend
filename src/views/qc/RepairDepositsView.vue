...
watch([search, tailorFilter], () => debounce(() => fetchData(1)))
...
<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      <div>
        <h1 class="text-2xl font-bold text-surface-900">{{ t('repairDeposits.title') }}</h1>
        <p class="mt-1 text-sm text-surface-500">{{ t('repairDeposits.description') }}</p>
      </div>
    ...
    <div class="flex flex-col sm:flex-row gap-3 mb-4">
      <div class="sm:w-48">
        <SearchableDropdown v-model="tailorFilter" :options="filterTailors" label="" :placeholder="t('repairDeposits.selectTailor')" clearable />
      </div>
      <div class="flex-1">
        <Input v-model="search" label="" :placeholder="t('repairDeposits.searchPlaceholder')" />
      </div>
    </div>
