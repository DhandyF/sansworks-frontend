...
async function confirmPurge() {
  ...
  try {
    await request(`/pre-orders/${id}/purge`, { method: 'POST' })
    const page = trashedItems.value.length === 1 && trashedPage.value > 1 ? trashedPage.value - 1 : trashedPage.value
    await fetchTrashed(page)
  } catch { /* ignore */ }
...
<template>
  <div>
    ...
    <Modal v-model="showPurgeModal" :title="t('preOrders.purge')" size="sm" :closeOnOverlay="false">
      <p class="text-surface-700">{{ t('preOrders.confirmPurge') }}</p>
      <template #footer>
        <div class="flex justify-end gap-3">
          <Button variant="outline" @click="showPurgeModal = false">{{ t('common.cancel') }}</Button>
          <Button variant="danger" @click="confirmPurge">{{ t('preOrders.purge') }}</Button>
        </div>
      </template>
    </Modal>
