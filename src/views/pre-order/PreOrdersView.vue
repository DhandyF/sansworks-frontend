...
const { items, loading, pagination, fetchData, addItem, editItem, deleteItem } = useMasterData('/pre-orders')
...
function openEditForm(item) {
  ...
  form.value = {
    ...
    items: [{ size_id: item.size_id, total_pcs: item.total_pcs }],
  }
  formError.value = ''
  fetchOptions()
  fetchArticles(item.brand_id)
  showForm.value = true
}
...
async function handleSubmit() {
  ...
  try {
    if (editing.value) {
      await editItem(editing.value.id, {
        brand_id: form.value.brand_id,
        article_id: form.value.article_id,
        size_id: form.value.items[0].size_id,
        total_pcs: Number(form.value.items[0].total_pcs),
      })
    } else {
...
<template>
  <div>
    ...
    <Card variant="bordered">
      ...
      <template v-else>
        <Table :columns="columns" :rows="items" :pagination="pagination" @page-change="fetchData">
          ...
          <template #size="{ row }">
            <Badge variant="default" size="sm">{{ row.size?.abbreviation || '-' }}</Badge>
          </template>
    ...
    <Modal v-model="showForm" :title="editing ? 'Edit Pre-Order' : 'Add Pre-Order'" size="lg" min-height="80vh">
      <div class="space-y-4">
        ...
        <div>
          <div class="flex items-center justify-between mb-2">
            ...
            <button v-if="!editing" @click="addItemRow" type="button" class="inline-flex items-center gap-1 text-sm text-primary-600 hover:text-primary-700 font-medium cursor-pointer">
          ...
          <div class="space-y-3">
            <div v-for="(item, index) in form.items" :key="index" class="grid grid-cols-1 md:grid-cols-2 gap-3 items-end">
              ...
              <div class="flex items-end gap-2">
                ...
                <button v-if="!editing && form.items.length > 1" @click="removeItemRow(index)" type="button" class="p-2 mb-0.5 rounded-lg text-red-500 hover:bg-red-50 transition-colors cursor-pointer" title="Remove">
